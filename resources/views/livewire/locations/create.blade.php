<div>
    <x-button :text="__('Create New Location')" wire:click="$toggle('modal')" sm />

    <x-modal :title="__('Create New Location')" wire x-on:open="setTimeout(() => $refs.company_name.focus(), 250)">
        <form id="location-create" wire:submit="save" class="space-y-4">
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <x-input label="{{ __('Company Name') }} *" x-ref="company_name" wire:model="location.company_name"
                        required />
                </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div>
                    <x-toggle label="{{ __('Residential Address') }} *" wire:model="location.address_residential_indicator" />
                </div>
            </div>
